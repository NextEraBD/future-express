...
class AttendanceDevice(models.Model):
    ...
    def action_restart(self):
        self.ensure_one()
        self.restartDevice()
        return self.message_box_show(_('Restart command has been send to the device %s. Please wait for a while to continue...') % (self.display_name,))
    ...
    def action_clear_data(self):
        ...
        try:
            ...
            self.disableDevice()
            self.zk.clear_data()
            return self.message_box_show(_('Clear Data from the device %s successfully!') % (self.display_name,))
        except Exception as e:
            _logger.error(e)
    ...
    def action_show_time(self):
        ...
        """
        self.ensure_one()
        return self.message_box_show(_("The Machine time is %s") % self.getMachineTime())
    ...
    def action_user_download(self):
        ...
        return self.message_box_show(_('Download Users from the device %s successfully!') % (self.display_name,))
    ...
    def action_user_upload(self):
        ...
        return self.message_box_show(_('Upload Users to the device %s successfully!') % (self.display_name,))
    ...
    def action_employee_map(self):
        ...
        return self.message_box_show(_('Map Odoo Employee and Device Users from the device %s successfully!') % (self.display_name,))
            ...
            if r.auto_clear_attendance_schedule == 'on_download_complete':
                ...
                if int(r.auto_clear_attendance_dow) == -1 or dt_now.weekday() == int(r.auto_clear_attendance_dow):
                    ...
                    if abs(delta) <= 0.5 or abs(delta) >= 23.5:
                        r.action_attendance_clear()
        return self.message_box_show(_('Download Attendance Data from the device %s successfully!') % (self.display_name,))
        ...
        for r in self:
            ...
            for template in template_data:
                ...
                if (str(template.uid) + '_' + str(template.fid)) not in existing_finger_template_ids:
                    ...
                    if existing:
                        existing.write(vals)
        return self.message_box_show(_('Download Finger Templates from the device %s successfully!') % (self.display_name,))
    ...
    def message_box_show(self, message):
        self.env['display.dialog.box'].search([]).unlink()
        dialog_id = self.env['display.dialog.box'].create({'text': message})
        return {
    ...
    def action_check_connection(self):
        ...
        if self.connect():
            self.disconnect()
            return self.message_box_show(_('Connect to the device %s successfully') % (self.display_name,))
    ...
    def action_device_information(self):
        ...
        for r in self:
            try:
                ...
                message = self.message_box_show(_('Getting information from the device %s successfully') % (self.display_name,))
                cr.commit()
                cr.close()
                return message
