...
class AttendanceSheetBatch(models.Model):
    ...
    department_id = fields.Many2many('hr.department', string='Department Name')
    ...
    @api.onchange('department_id', 'date_from', 'date_to', 'all_company')
    def onchange_employee(self):
        if not self.all_company:
            ...
            self.name = _('Attendance Batch of %s  Departments for %s') % (
                department_names,
                tools.ustr(
    ...
    def gen_att_sheet(self):
        ...
        for batch in self:
            ...
            if batch.all_company:
                employee_ids = self.env['hr.employee'].search(
                    [('company_id', '=', self.env.company.id)])
            else:
                employee_ids = self.env['hr.employee'].search(
                    [('department_id', 'in', batch.department_id.ids)])
