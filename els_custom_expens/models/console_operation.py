...
class Console(models.Model):
    ...
    def default_get(self, default_fields):
        res = super(Console, self).default_get(default_fields)
    ...
    def compute_claim_counter(self):
        for rec in self:
            rec.claim_counter = self.env['account.move'].search_count([('console_id', '=', rec.id), ('move_type','=','out_invoice')])
    def compute_entry_counter(self):
        for rec in self:
            rec.entry_counter = self.env['account.move'].search_count([('console_id', '=', rec.id), ('move_type','=','entry')])

    def compute_event_counter(self):
        for rec in self:
            rec.event_counter = self.env['els.events'].search_count([('console_id', '=', rec.id), ('state','=','submit')])
    ...
    def action_create_claim(self):
        ...
        if official_lines:
            ...
            if self.company_id.id == 1:
                journal_id = self.env.ref('custom_hr_expense.els_claim_customer_journal').id
            elif self.company_id.id == 2:
                journal_id = self.env.ref('custom_hr_expense.etal_claim_customer_journal').id
