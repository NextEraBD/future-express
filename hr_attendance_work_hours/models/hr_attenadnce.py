            ...
            if calendar and rec.check_in and rec.check_out:
                ...
                for day in days_from_hours:
                    ...
                    work_day_start = user_timezone.localize(
                        rec.check_in.replace(hour=in_hour, minute=in_min)).astimezone(timezone('UTC'))
                    ...
                    matched_recs1 = self.search([('check_in', '>', check_out1),
                                                 ('check_out', '<=', day_end),
                                                 ('employee_id', '=', rec.employee_id.id)], order='check_in asc')
                    leave = rec.employee_id._get_leave_days_data_batch(work_day_start, local_date, calendar=calendar)
                    if day.hour_from < hours_converted and out_hour > hours_converted and matched_recs and rec.id == \
                            matched_recs.ids[0]:
                        late_mins = hours_converted - day.hour_from
                        grace_period = day.grace_period
                    elif day.hour_from < hours_converted and out_hour > hours_converted and matched_recs1 and rec.id == \
                            matched_recs1.ids[0] and not prev_shift:
                        late_mins = hours_converted - day.hour_from
                        grace_period = day.grace_period
                ...
                for day in days_to_hours:
                    ...
                    if not att_after_checkout:
                        ...
                        leave = rec.employee_id._get_leave_days_data_batch(local_date_to, work_day_end,
                                                                           calendar=calendar)
